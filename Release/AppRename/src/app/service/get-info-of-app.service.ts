...
export class GetInfoOfAppService {
  ...
  getInfos_service(page: string) {
    ...
    return this.http.post('/sql', o_post);
  }
  ...
  postInfos_service(theme: any, reqBody: any) {
    ...
    return this.http.post('/sql', o_post); // 后台使用req.body.page取到该值。这里不设置{headers: new HttpHeaders({'Content-Type': 'application/x-www-form-urlencoded'})}
  }
  ...
  postSetThemeName(reqBody: string) {
    ...
    return this.http.post('/theme', o_theme_name); // 后台使用req.body.page取到该值。这里不设置{headers: new HttpHeaders({'Content-Type': 'application/x-www-form-urlencoded'})}
  }
  ...
  postQueryApps(reqBody: string) {
    ...
    return this.http.post('/query', o_query_keyword); // 后台使用req.body.page取到该值。这里不设置{headers: new HttpHeaders({'Content-Type': 'application/x-www-form-urlencoded'})}
  }
  ...
  getAppsByTheme_service(reqBody: any, page: any) {
    ...
    return this.http.get('/theme', {params: theme});
  }
